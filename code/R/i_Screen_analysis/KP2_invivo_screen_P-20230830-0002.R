...
# https://biodoc.pri.bms.com:8443/display/MGS/P-20230830-0002-2023-Q1-KP2-Invivo-screen_Double-Input
...
# User-defined genes of interest for plotting
genes_interest <- c("Met", "Cd109", "Pmepa1")
...
file_log <- paste0(output_path_result, "log.txt")
...
create_violin_plot_my(lib_size_pipe_df %>%
                      ...
                      x_label="SampleCategory", y_label="P-20230830-0002", fill_label="SampleCategory", 
                      filename=filename, labels=c("Sample Category", "Guide Number", "P-20230830-0002"))
...
# Make sure that DepMap data still only has one row per symbol
# That could have changed during the homology translation
depmap_df <- depmap_df %>% 
  dplyr::group_by(Symbol) %>% 
  dplyr::summarise(DepMapStatus = max(DepMapStatus), 
                   KPC_Dep = paste(KPC_Dep, collapse=";"), 
                   DepStatus = paste(DepStatus, collapse=";"))
...
for(pair in pair_list){
  ...
  for(f in c("Significant", "GeneCategory")){  # "DepStatus"
...
# Plot boxplot of normalized levels for select genes
for(s in genes_interest){
  
  if(s %in% screen_norm_pdna_long_df$Symbol){
    filename <- paste0(output_path_result, "box_normlevels_user_", s)
    p <- create_box_plot_my(screen_norm_pdna_long_df %>%
                              dplyr::filter(Symbol == s), 
                            x_label="SampleCategory", y_label="GuideNormLogpDNA", fill_label="sgRNA",
                            filename=NULL, labels=c("Sample Group", "Guide levels, log norm", "Guide levels by group"),
